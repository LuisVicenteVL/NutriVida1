...
class PacientesController
{
    ...
    static async indexGet(req, res) {
        let data = await PacientesModel.consultar();
        res.send(data);
    }
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await PacientesModel.insertar(newData);

            res.status(201)
                .header('Location', `/Pacientes/${insertedId}`)
    ...
    static async itemGet(req, res) {
        ...
        let data = await PacientesModel.consultarPorId(id);
        if (data.length == 0) {
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await PacientesModel.actualizar(id, updatedFields);
