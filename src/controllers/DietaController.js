...
class DietaController
{
    ...
    static async indexGet(req, res) {
        let data = await DietaModel.consultar();
        res.send(data);
    }
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await DietaModel.insertar(newData);

            res.status(201)
                .header('Location', `/Dieta/${insertedId}`)
    ...
    static async itemGet(req, res) {
        ...
        let data = await DietaModel.consultarPorId(id);
        if (data.length == 0) {
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await DietaModel.actualizar(id, updatedFields);
