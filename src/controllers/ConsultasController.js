...
class ConsultasController
{
    ...
    static async indexGet(req, res) {
        let data = await ConsultasModel.consultar();
        res.send(data);
    }
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await ConsultasModel.insertar(newData);

            res.status(201)
                .header('Location', `/Consultas/${insertedId}`)
    ...
    static async itemGet(req, res) {
        ...
        let data = await ConsultasModel.consultarPorId(id);
        if (data.length == 0) {
    ...
    static async itemPatch(req, res) {
        try {
            ...
            const result = await ConsultasModel.actualizar(id, updatedFields);
